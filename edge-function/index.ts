...
async function handleEnqueue(req: Request): Promise<Response> {
  ...
  const { data: job, error: insErr } = await supabase
    ...
    .select('id')
    .single()
  if (insErr || !job) return json({ error: 'enqueue_failed', detail: insErr?.message }, 500)
...
async function handleStatus(url: URL): Promise<Response> {
  ...
  if (!jobId || !UUID_RE.test(jobId)) return json({ error: 'invalid_job_id' }, 400)
...
async function maybeReloginPayload(status: string | null | undefined) {
  ...
  return { relogin_url: WORKER_PUBLIC_URL ? `${WORKER_PUBLIC_URL}/relogin` : null }
}
